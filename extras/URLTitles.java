...
public class URLTitles {
	...
	public static String find(String s){
		return findYoutube(s);
	}
	
	public static String findYoutube(String s){
		String videoid = "none";
		
		if(s.contains("youtu.be")){
			videoid = s.split("youtu.be/")[1];
		}
		else{
		...
		try {
			...
			Pattern daylong = Pattern.compile("PT*D*H*M*S");
			Pattern hourlong = Pattern.compile("PT*H*M*S");
			Pattern minlong = Pattern.compile("PT*M*S");
			Pattern seclong = Pattern.compile("PT*S");
			String dur;
            ...
            double percentlike;
            if(statistics.get("likeCount").getAsInt() == 0){
            	percentlike = 0;
            }
